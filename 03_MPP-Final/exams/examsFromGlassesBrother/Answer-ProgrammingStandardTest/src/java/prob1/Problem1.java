...
public class Problem1 {
	...
	public static List<String> elementsInJustOne(List<String> list1, List<String> list2) {
	    List<String> result = Stream.concat(list1.stream().filter(str1->!list2.contains(str1)),list2.stream().filter(str2->!list1.contains(str2))).collect(Collectors.toList());
		return result;
	}
	...
	public static List<String> getAllFairfieldCustomers(List<Customer> list) {
	    List<String> cusName = list.stream().filter(cus -> cus.getCity().equals("Fairfield"))
	                                        .map(cus -> cus.getName())
	                                        .sorted()
